...
struct SynchronizedGateRunner<T: Clone> {
    condition: NodeRunnerInputPort<bool>,
...
impl<T: Clone> NodeRunner for SynchronizedGateRunner<T> {
    fn run(self: Box<Self>) {
        loop {
            let condition = self.condition.recv().unwrap();
...
pub struct SynchronizedGate<T: Clone> {
    pub condition: NodeConfigInputPort<bool>,
...
impl<T: Clone> SynchronizedGate<T> {
    pub fn new() -> Self {
        Self {
            condition: NodeConfigInputPort::new(),
            input: NodeConfigInputPort::new(),
...
impl<T: Clone> Default for SynchronizedGate<T> {
    fn default() -> Self {
        Self::new()
    }
}
...
// TODO: Can + Send + 'static be removed?
impl<T: Clone + Send + 'static> NodeConfig for SynchronizedGate<T> {
    fn into_runner(self: Box<Self>) -> Box<dyn NodeRunner + Send> {
        Box::new(SynchronizedGateRunner {
            condition: self.condition.into(),
            input: self.input.into(),
            output: self.output.into(),
        })
